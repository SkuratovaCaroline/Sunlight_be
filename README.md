...
![ER-диаграмма базы данных](docs/images/er-diagram.jpg)
...
| Метод | URL | Описание | Роль |
|-------|-----|-----------|------|
|PUT | `/api/cinema/users/{id}/avatar` | обновить/добавить аватарку пользователя |
|DELETE | `/api/cinema/users/{id}/avatar` | удалить аватарку пользователя |
|GET | `/api/cinema/users/{id}/avatar`| получить путь к аватарке пользователя |

### Фильмы
PUT | `/api/cinema/films/{id}/poster` | добавить/обновить постер фильма |
GET | `/api/cinema/films/genre/{genre}` | фильтрация фильмов по жанру|
GET | `/api/cinema/films/{id}/rating` | получить среднюю оценку фильма|

### Билеты
| Метод | URL | Описание | Роль |
|-------|-----|-----------|------|
|POST| `/api/cinema/tickets/purchase` | покупка билета (передаются userId, sessionId, seatId, price)|
|GET| `/api/cinema/users/{id}/tickets/past` | получить билеты пользователя на прошедшие сеансы|
|GET| `/api/cinema/users/{id}/tickets/future` |  получить билеты пользователя на будущие сеансы|

### Расписание
| Метод | URL | Описание | Роль |
|-------|-----|-----------|------|
|GET | `/api/cinema/films/{id}/schedule` |  получить расписание сеансов для конкретного фильма|

### Места
| GET| `/api/cinema/sessions/{id}/seats` | получить занятые и свободные места для конкретного сеанса|

### Отзывы
|POST | `/api/cinema/films/{id}/reviews` | создать новый отзыв на фильм (параметры: userId, text)|
|GET| `/api/cinema/films/{id}/reviews` |  получить список отзывов для фильма|

### Рейтинги
|POST | `/api/cinema/films/{id}/ratings`|  создать или обновить оценку фильма пользователем (параметры: userId, rating)|

### Уведомления
|GET| `/api/cinema/users/{id}/notifications`| получить все уведомления пользователя|
